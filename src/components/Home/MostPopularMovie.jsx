...
const MostPopularMovie = () => {
    const { movie, error, loading } = useMostPopularMovie();
    ...
    return (
        <section
            ...
            bg-[url('https://image.tmdb.org/t/p/original${movie.poster_path}')] bg-cover bg-no-repeat bg-fixed bg-center
            ...
            sm:bg-[url('https://image.tmdb.org/t/p/original${movie.backdrop_path}')]`}>
            <section className="w-full absolute bottom-40 left-1/2 -translate-x-1/2 z-40 sm:bottom-20">
                ...
                <Link to={`/movie/${movie.id}`} className="bg-red-600 text-white text-xl font-raleway font-bold block w-fit mx-auto my-4 px-6 py-1 rounded-sm shadow-xl lg:text-2xl">
                    Ver Mais
                </Link>
            </section>
        </section>
